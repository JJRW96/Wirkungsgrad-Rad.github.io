...
Erg_data_short <- Erg_data_komplett[, c("Proband", "Nr", "Bedingung", "Intensität", "Masse", 
                                       "P_mech", "P_mech_Erg_avg", "P_mech_Vorgabe", "nD", "nD_Vorgabe",
                                       "lKurbel", "Koerperlaenge", "q", "Wint_Umdrehung", "Wint_ges_kJ",
                                       "P_Int_Min", "P_Tot_Min", "lBein", "lOS", "lUS",
                                       "uOS", "uUS", "Faktor_Used", "P_mean", "P_mean_abs",
                                       "W_kg", "W_abs_kg", "P_Sum", "P_negSum", "P_posSum",
                                       "Efficiency", "P_max", "P_max_kg", "Pedal_Smoothness", "P_L_percent",
                                       "P_R_percent", "PInt_Kinematik", "PInt_Modell", "PInt_Kinematik_Modell")]

names(Erg_data_short)[names(Erg_data_short) == "Masse"] <- "Masse [kg]"
names(Erg_data_short)[names(Erg_data_short) == "P_mech"] <- "P_mech [W]"
names(Erg_data_short)[names(Erg_data_short) == "P_mech_Erg_avg"] <- "P_mech_Erg_avg [W]"
...
names(Erg_data_short)[names(Erg_data_short) == "P_Int_Min"] <- "P_Int_Min [W]"
names(Erg_data_short)[names(Erg_data_short) == "P_Tot_Min"] <- "P_Tot_Min [W]"
...
names(Erg_data_short)[names(Erg_data_short) == "uOS"] <- "uOS [kg]"
names(Erg_data_short)[names(Erg_data_short) == "uUS"] <- "uUS [kg]"
names(Erg_data_short)[names(Erg_data_short) == "P_mean"] <- "P_mean [W]"
names(Erg_data_short)[names(Erg_data_short) == "P_mean_abs"] <- "P_mean_abs [W]"
names(Erg_data_short)[names(Erg_data_short) == "W_kg"] <- "W_kg [W·kg⁻¹]"
names(Erg_data_short)[names(Erg_data_short) == "W_abs_kg"] <- "W_abs_kg [W·kg⁻¹]"
names(Erg_data_short)[names(Erg_data_short) == "P_Sum"] <- "P_Sum [W]"
names(Erg_data_short)[names(Erg_data_short) == "P_negSum"] <- "P_negSum [W]"
names(Erg_data_short)[names(Erg_data_short) == "P_posSum"] <- "P_posSum [W]"
names(Erg_data_short)[names(Erg_data_short) == "Efficiency"] <- "Efficiency [%]"
...
names(Erg_data_short)[names(Erg_data_short) == "PInt_Kinematik"] <- "PInt_Kinematik [W]"
names(Erg_data_short)[names(Erg_data_short) == "PInt_Modell"] <- "PInt_Modell [W]"
names(Erg_data_short)[names(Erg_data_short) == "PInt_Kinematik_Modell"] <- "PInt_Kinematik_Modell [W]"
...
```{r}
# Filtern der Daten für "sitzen"
sitzen_data <- Bedingungen_data[Bedingungen_data$Bedingung == "sitzen", ]

# Lineare Regression mit gefilterten Daten
lin_reg <- lm(Efficiency ~ Wirk_Netto, data = sitzen_data)
reg_coefficients <- coef(lin_reg)
r_squared <- summary(lin_reg)$r.squared
equation_text <- sprintf("y = %.2fx + %.2f", reg_coefficients["Wirk_Netto"], reg_coefficients["(Intercept)"])
r_squared_text <- sprintf("R² = %.2f", r_squared)
summary_lin_reg <- summary(lin_reg)
f_stat <- summary_lin_reg$fstatistic
p_value <- format.pval(pf(f_stat["value"], f_stat["numdf"], f_stat["dendf"], lower.tail = FALSE), digits = 4)
...
# Berechnung der Regressionsgeraden für eine Sequenz von Wirk_Netto
Wirk_Netto_seq <- seq(min(sitzen_data$Wirk_Netto), max(sitzen_data$Wirk_Netto), length.out = 100)
regression_values <- predict(lin_reg, newdata = data.frame(Wirk_Netto = Wirk_Netto_seq))

# Erstellen des Plotly-Diagramms
plot_ly() %>%
  add_markers(data = sitzen_data, x = ~Wirk_Netto, y = ~Efficiency, type = 'scatter', mode = 'markers',
              color = ~factor(Proband), colors = colorRampPalette(brewer.pal(10,"Spectral"))(20),
              marker = list(size = 9, opacity = 0.8)) %>%
  layout(title = "Efficiency vs. Wirk_Netto (Sitzen)",
         margin = list(t = 40), 
         xaxis = list(title = "Wirk_Netto"),
         yaxis = list(title = "Efficiency")) %>%
  add_lines(x = ~Wirk_Netto_seq, y = ~regression_values, name = "Regressionslinie", line = list(color = 'darkgrey', width = 2)) %>%
  add_annotations(text = equation_text, x = min(sitzen_data$Wirk_Netto), y = max(sitzen_data$Efficiency), 
                  showarrow = FALSE, xanchor = 'left', yanchor = 'bottom') %>%
  add_annotations(text = r_squared_text, x = min(sitzen_data$Wirk_Netto), y = max(sitzen_data$Efficiency) * 0.98, 
                  showarrow = FALSE, xanchor = 'left', yanchor = 'bottom') %>%
  add_annotations(text = f_stat_text, x = min(sitzen_data$Wirk_Netto), y = max(sitzen_data$Efficiency) * 0.96, 
                  showarrow = FALSE, xanchor = 'left', yanchor = 'bottom')
```

```{r}
# Filtern der Daten für "stehen"
sitzen_data <- Bedingungen_data[Bedingungen_data$Bedingung == "stehen", ]
