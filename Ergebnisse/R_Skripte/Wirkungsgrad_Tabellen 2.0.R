...
Bedingungen_data_Wirkungsgrad <- Bedingungen_data[, c("Proband", "Bedingung", "Intensität", "Wirk_Muskulär", "W*Aerob [kJ]", "WPCR [kJ]", "WBLC [kJ]", "WTOT [kJ]", "P_Tot", "P_mean", "P_Int", "nD", "Efficiency", "Pedal_Smoothness", "P_max", "HR_percent", "VO2_percent", "ΔBLC", "Wirk_Brutto", "Wirk_Netto","Wirk_Total", "Wirk_Arbeit")]
...
names(Bedingungen_data_Wirkungsgrad)[names(Bedingungen_data_Wirkungsgrad) == "Wirk_Total" ] <- "Wirk_Total [%]"
...
stats <- data.frame(
  Variable = c("Wirk_muskulär [%]", "Wirk_Total [%]", "Wirk_Netto [%]", "Wirk_Brutto [%]", "Wirk_Arbeit [%]",
...
stats$`Sitzen (MW ± SD)` <- sapply(stats$Variable, function(var) {
  ...
  if(var %in% c("Wirk_muskulär [%]","Wirk_Total [%]", "Wirk_Netto [%]", "Wirk_Brutto [%]", 
    ...
    if(normal_dist && var_homogen) {
      # Parametrischer Test (t-Test)
      sit_values <- temp_data$dv[temp_data$Bedingung == "sitzen"]
      stand_values <- temp_data$dv[temp_data$Bedingung == "stehen"]
      t_test_result <- t.test(sit_values, stand_values, paired = TRUE)
      p_value <- t_test_result$p.value
      
      # Berechnung Cohen's d für gepaarte Stichproben
      diff_values <- sit_values - stand_values
      d <- mean(diff_values) / sd(diff_values)
      ...
      test_message <- paste0(test_message,
                             ...
                             sprintf("Cohen's d = %.3f (%s)\n", 
                                     d, effect_interpretation),
                             "\nVerwendeter Test: Gepaarter t-Test\n")
    } else {
...
stats$`Stehen (MW ± SD)` <- sapply(stats$Variable, function(var) {
  ...
  if(var %in% c("Wirk_muskulär [%]","Wirk_Total [%]", "Wirk_Arbeit [%]","Wirk_Brutto [%]", "Wirk_Netto [%]")) {
...
ft_Wirkungsgrad_stats_Bedingung_mean <- flextable(stats) %>%
  ...
  compose(
    j = "Parameter",
    i = ~ Parameter == "Wirk_Total [%]",
    value = as_paragraph("η", as_sub("Total"), " [%]", as_sup(" b"))
  ) %>%
  compose(
    j = "Parameter",
...
posthoc_ergebnisse_Intensität_Wirk_muskulär <- NULL
posthoc_ergebnisse_Intensität_Wirk_Brutto <- NULL
...
stats$`Leicht (MW ± SD)` <- sapply(stats$Variable, function(var) {
  ...
  if(var %in% c("Wirk_muskulär [%]","Wirk_Total [%]", "Wirk_Netto [%]", "Wirk_Brutto [%]")) {  
    ...
    } else if(var == "Wirk_Total [%]") {
      temp_data$dv <- temp_data$`Wirk_Total [%]` * 100
      current_data_light <- data_light$`Wirk_Total [%]`
      current_data_moderate <- data_moderate$`Wirk_Total [%]`
      current_data_heavy <- data_heavy$`Wirk_Total [%]`
    }
    ...
    if(normal_dist && var_homogen) {
      ...
      mauchly_result <- ez_result$`Mauchly's Test for Sphericity`
      if(!is.null(mauchly_result)) {
        mauchly_w <- mauchly_result$W[1]
        mauchly_p <- mauchly_result$p[1]
        sphericity_assumed <- mauchly_p > 0.05
        
        # GG und HF Korrekturen
        gg_epsilon <- ez_result$`Sphericity Corrections`$GG[1]
        hf_epsilon <- ez_result$`Sphericity Corrections`$HF[1]
        
        print(sprintf("\n--- Mauchly-Test auf Sphärizität für %s ---", var))
        print(sprintf("%s: W = %.3f, p = %.4f", var, mauchly_w, mauchly_p))
        print(sprintf("%s: Greenhouse-Geisser ε = %.3f", var, gg_epsilon))
        print(sprintf("%s: Huynh-Feldt ε = %.3f", var, hf_epsilon))
        
        # Korrigierte p-Werte wenn nötig
        if(!sphericity_assumed) {
          if(gg_epsilon < 0.75) {
            # Greenhouse-Geisser Korrektur
            corrected_df_effect <- df_effect * gg_epsilon
            corrected_df_error <- df_error * gg_epsilon
            corrected_p <- ez_result$`Sphericity Corrections`$`p[GG]`
            p_value <- corrected_p  # Hier wird p_value aktualisiert
            print(sprintf("F(%.3f,%.3f) = %.3f (Greenhouse-Geisser korrigiert)", 
                          corrected_df_effect, corrected_df_error, f_value))
            print(sprintf("p-Wert = %.4f (Greenhouse-Geisser korrigiert)", corrected_p))
          } else {
            # Huynh-Feldt Korrektur
            corrected_df_effect <- df_effect * hf_epsilon
            corrected_df_error <- df_error * hf_epsilon
            corrected_p <- ez_result$`Sphericity Corrections`$`p[HF]`
            p_value <- corrected_p  # Hier wird p_value aktualisiert
            print(sprintf("F(%.3f,%.3f) = %.3f (Huynh-Feldt korrigiert)", 
                          corrected_df_effect, corrected_df_error, f_value))
            print(sprintf("p-Wert = %.4f (Huynh-Feldt korrigiert)", corrected_p))
          }
        } else {
      ...
      if(var == "Wirk_muskulär [%]" || var == "Wirk_Brutto [%]") {
        # Extrahieren der Daten für jede Intensität
        ...
        # Durchführung der gepaarten t-Tests
        ...
        # Bonferroni-Korrektur (multipliziere p-Werte mit 3)
        p_adj <- c(t_ml$p.value, t_sl$p.value, t_sm$p.value) * 3
        
        # Erstellen der Ergebnis-Matrix
        post_hoc_matrix <- matrix(
          c(mean(moderat_data - leicht_data),
            mean(schwer_data - leicht_data),
            mean(schwer_data - moderat_data),
            t_ml$conf.int[1], t_sl$conf.int[1], t_sm$conf.int[1],
            t_ml$conf.int[2], t_sl$conf.int[2], t_sm$conf.int[2],
            p_adj,
            c(cohens_d_light_mod, cohens_d_light_heavy, cohens_d_mod_heavy)),
          nrow = 3,
          ncol = 5,
          dimnames = list(
            c("moderat-leicht", "schwer-leicht", "schwer-moderat"),
            c("diff", "lwr", "upr", "p adj", "Cohen's d")
          )
        )
        
        if(var == "Wirk_muskulär [%]") {
          posthoc_ergebnisse_Intensität_Wirk_muskulär <<- list(Intensität = post_hoc_matrix)
        } else if(var == "Wirk_Brutto [%]") {
          posthoc_ergebnisse_Intensität_Wirk_Brutto <<- list(Intensität = post_hoc_matrix)
        }
      }
    }
    ...
    # Post-hoc Ergebnisse
    if(var == "Wirk_muskulär [%]") {
      print("\n--- Post-hoc Analyse Wirk_muskulär ---")
      print("Paarweise Vergleiche:")
      print(posthoc_ergebnisse_Intensität_Wirk_muskulär)
    } else if(var == "Wirk_Brutto [%]") {
      print("\n--- Post-hoc Analyse Wirk_Total ---")
      print("Paarweise Vergleiche:")
      print(posthoc_ergebnisse_Intensität_Wirk_Brutto)
    }
...
if(!is.null(mauchly_result)) {
  ...
  print("\n--- Mauchly-Test auf Sphärizität ---")
  print(sprintf("W = %.3f, p = %.4f", mauchly_w, mauchly_p))
  print(sprintf("Greenhouse-Geisser ε = %.3f", gg_epsilon))
  print(sprintf("Huynh-Feldt ε = %.3f", hf_epsilon))
  ...
  if(!sphericity_assumed) {
    if(gg_epsilon < 0.75) {
      ...
      corrected_df_effect <- df_effect * hf_epsilon
      corrected_df_error <- df_error * hf_epsilon
      print("\nHuynh-Feldt Korrektur verwendet")
    }
    # Aktualisierte F-Statistik mit korrigierten Freiheitsgraden
    print(sprintf("Korrigiert: F(%.2f,%.2f) = %.3f, p = %.4f", 
                  corrected_df_effect, corrected_df_error, f_value, 
                  ez_result$`Sphericity Corrections`$p[if(gg_epsilon < 0.75) 1 else 2]))
  }
}
...
cohens_d <- function(x, y) {
  diff <- x - y
  d <- abs(mean(diff, na.rm = TRUE) / sd(diff, na.rm = TRUE))
  return(d)
}
...
ft_Wirkungsgrad_stats_Intensitaet_mean <- flextable(stats) %>%
  ...
  compose(
    j = "Parameter",
    i = ~ Parameter == "Wirk_Total [%]",
    value = as_paragraph("η", as_sub("Total"), " [%]", as_sup(" a"))
  ) %>%
  compose(
    j = "Parameter",
    i = ~ Parameter == "Wirk_Arbeit [%]",
  ...
  add_footer_row(
    values = as_paragraph(
      ...
      as_sup("†"), " : Signifikanter Effekt der Intensität (p < .05). \n",
      "η", as_sub("muskulär"), " [%]: muskulärer Wirkungsgrad; ",
      "η", as_sub("Total"), " [%]: Gesamtwirkungsgrad; ",
      "η", as_sub("Arbeit,sitzen"), " [%]: Arbeitswirkungsgrad (sitzende Position); ",
      "η", as_sub("Netto"), " [%]: Nettowirkungsgrad; ",
...
post_hoc_data <- data.frame(
  Vergleich = rownames(posthoc_ergebnisse_Intensität_Wirk_muskulär$Intensität),
  Differenz = posthoc_ergebnisse_Intensität_Wirk_muskulär$Intensität[, "diff"],
  KI_unten = posthoc_ergebnisse_Intensität_Wirk_muskulär$Intensität[, "lwr"],
  KI_oben = posthoc_ergebnisse_Intensität_Wirk_muskulär$Intensität[, "upr"],
  p_Wert = posthoc_ergebnisse_Intensität_Wirk_muskulär$Intensität[, "p adj"],
  Cohens_d = posthoc_ergebnisse_Intensität_Wirk_muskulär$Intensität[, "Cohen's d"]
)
...
ft_post_hoc_Intensität_Wirk_muskulär <- flextable(post_hoc_data) %>%
  set_header_labels(
    Vergleich = " ",
    Differenz = "Mittlere Differenz",
    KI_unten = "95% KI Untergrenze",
    KI_oben = "95% KI Obergrenze",
    p_Wert = "p-Wert (Bonf.-korr.)",
...
# Setzen der Tabelle auf volle Breite
ft_post_hoc_Intensität_Wirk_muskulär <- set_table_properties(
...
# Speichern in ...Probanden_Energieberechnung/xlsm
#saveRDS(ft_post_hoc_Intensität_Wirk_muskulär, "C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Daten/Probanden_Energieberechnung/xlsm/ft_post_hoc_Intensität_Wirk_muskulär.rds")
#ft_post_hoc_Intensität_Wirk_muskulär <- readRDS("C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Daten/Probanden_Energieberechnung/xlsm/ft_post_hoc_Intensität_Wirk_muskulär.rds")

# Speichern in ...Ergebnisse/rds
#saveRDS(ft_post_hoc_Intensität_Wirk_muskulär, "C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad.github.io/Ergebnisse/rds/ft_post_hoc_Intensität_Wirk_muskulär.rds")
#ft_post_hoc_Intensität_Wirk_muskulär <- readRDS("C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad.github.io/Ergebnisse/rds/ft_post_hoc_Intensität_Wirk_muskulär.rds")

##################################################################################
##### Post-hoc für Intensitäten und Wirk_Brutto #####
# Post-hoc Daten mit Cohen's d
post_hoc_data_Wirk_Brutto <- data.frame(
  Vergleich = rownames(posthoc_ergebnisse_Intensität_Wirk_Brutto$Intensität),
  Differenz = posthoc_ergebnisse_Intensität_Wirk_Brutto$Intensität[, "diff"],
  KI_unten = posthoc_ergebnisse_Intensität_Wirk_Brutto$Intensität[, "lwr"],
  KI_oben = posthoc_ergebnisse_Intensität_Wirk_Brutto$Intensität[, "upr"],
  p_Wert = posthoc_ergebnisse_Intensität_Wirk_Brutto$Intensität[, "p adj"],
  Cohens_d = posthoc_ergebnisse_Intensität_Wirk_Brutto$Intensität[, "Cohen's d"]
)

# Neuordnung und Formatierung
post_hoc_data_Wirk_Brutto <- post_hoc_data_Wirk_Brutto[c(2, 1, 3), ]
post_hoc_data_Wirk_Brutto$Vergleich <- gsub("moderat", "Moderat", post_hoc_data_Wirk_Brutto$Vergleich)
post_hoc_data_Wirk_Brutto$Vergleich <- gsub("leicht", "Leicht", post_hoc_data_Wirk_Brutto$Vergleich)
post_hoc_data_Wirk_Brutto$Vergleich <- gsub("schwer", "Schwer", post_hoc_data_Wirk_Brutto$Vergleich)
...
# Signifikanzsymbole hinzufügen
post_hoc_data_Wirk_Brutto$p_Wert <- paste0(format(round(post_hoc_data_Wirk_Brutto$p_Wert, 3), nsmall = 3),
                               sapply(post_hoc_data_Wirk_Brutto$p_Wert, add_significance_symbols))

# Direkte Ausgabe der Post-hoc-Ergebnisse
cat("\nErgebnisse des Post-hoc-Tests:\n")
cat("----------------------------\n")

for(i in 1:nrow(post_hoc_data_Wirk_Brutto)) {
  p_val <- as.numeric(sub("[*]+$", "", post_hoc_data_Wirk_Brutto$p_Wert[i]))
  sig_level <- if(p_val < 0.05) "signifikant" else "nicht signifikant"
  
  cat(sprintf("%s:\n", post_hoc_data_Wirk_Brutto$Vergleich[i]))
  cat(sprintf("  Mittlere Differenz: %.2f\n", post_hoc_data_Wirk_Brutto$Differenz[i]))
  cat(sprintf("  p-Wert: %s (%s)\n", post_hoc_data_Wirk_Brutto$p_Wert[i], sig_level))
  cat("----------------------------\n")
}


# Erstellen der Flextable
ft_post_hoc_Intensität_Wirk_Brutto <- flextable(post_hoc_data_Wirk_Brutto) %>%
  set_header_labels(
    Vergleich = " ",
    Differenz = "Mittlere Differenz",
    KI_unten = "95% KI Untergrenze",
  ...
  compose(
    part = "header",  
    i = 1, j = "Vergleich",
    value = as_paragraph("Intensitäten: Post-hoc Vergleich von ", "η", as_sub("Brutto"))
  ) %>%
  theme_zebra(
    odd_header = "grey92",
    even_header = "grey92",
...
# Setzen der Tabelle auf volle Breite
ft_post_hoc_Intensität_Wirk_Brutto <- set_table_properties(
  ft_post_hoc_Intensität_Wirk_Brutto,  # Korrigiert
  width = 1, 
  layout = "autofit"
)
...
# Speichern in ...Probanden_Energieberechnung/xlsm
#saveRDS(ft_post_hoc_Intensität_Wirk_Brutto, "C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Daten/Probanden_Energieberechnung/xlsm/ft_post_hoc_Intensität_Wirk_Brutto.rds")
#ft_post_hoc_Intensität_Wirk_Brutto <- readRDS("C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Daten/Probanden_Energieberechnung/xlsm/ft_post_hoc_Intensität_Wirk_Brutto.rds")

# Speichern in ...Ergebnisse/rds
#saveRDS(ft_post_hoc_Intensität_Wirk_Brutto, "C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad.github.io/Ergebnisse/rds/ft_post_hoc_Intensität_Wirk_Brutto.rds")
#ft_post_hoc_Intensität_Wirk_Brutto <- readRDS("C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad.github.io/Ergebnisse/rds/ft_post_hoc_Intensität_Wirk_Brutto.rds")
...
#################################################################################
### Bedingung x Intensität
# `Wirk_muskulär [%]` `Wirk_Total [%]` `Wirk_Netto [%]`  `Wirk_Brutto [%]`

# 1. Prüfung der Normalverteilung für jede Kombination
normality_tests <- Bedingungen_data_Wirkungsgrad %>%
  group_by(Bedingung, Intensität) %>%
  summarise(
    shapiro_stat = shapiro.test(`Wirk_Brutto [%]` * 100)$statistic,
    shapiro_p = shapiro.test(`Wirk_Brutto [%]` * 100)$p.value
  )

print("Shapiro-Wilk Tests für Normalverteilung:")
print(normality_tests)

# 2. Levene-Test für Varianzhomogenität
levene_test <- car::leveneTest(`Wirk_Brutto [%]` * 100 ~ Bedingung * Intensität, 
                               data = Bedingungen_data_Wirkungsgrad)
print("\nLevene-Test für Varianzhomogenität:")
print(levene_test)

# 3. Zweifaktorielle ANOVA mit Messwiederholung
...
# Formatierung der ANOVA-Ergebnisse
format_anova_results <- function(anova_result) {
  anova_result$ANOVA$SSn <- round(anova_result$ANOVA$SSn, 4)
  anova_result$ANOVA$SSd <- round(anova_result$ANOVA$SSd, 4)
  anova_result$ANOVA$F <- round(anova_result$ANOVA$F, 2)
  anova_result$ANOVA$p <- round(anova_result$ANOVA$p, 3)
  anova_result$ANOVA$ges <- round(anova_result$ANOVA$ges, 3)
  
  anova_result$ANOVA$p[anova_result$ANOVA$p < .001] <- "< .001"
  
  if ("Sphericity Tests" %in% names(anova_result)) {
    anova_result$`Sphericity Tests`$W <- round(anova_result$`Sphericity Tests`$W, 3)
    anova_result$`Sphericity Tests`$p <- round(anova_result$`Sphericity Tests`$p, 3)
    anova_result$`Sphericity Tests`$p[anova_result$`Sphericity Tests`$p < .001] <- "< .001"
  }
  
  if ("Sphericity Corrections" %in% names(anova_result)) {
    anova_result$`Sphericity Corrections`$GGe <- round(anova_result$`Sphericity Corrections`$GGe, 3)
    anova_result$`Sphericity Corrections`$HFe <- round(anova_result$`Sphericity Corrections`$HFe, 3)
    
    if ("p[GG]" %in% names(anova_result$`Sphericity Corrections`)) {
      anova_result$`Sphericity Corrections`$`p[GG]` <- round(anova_result$`Sphericity Corrections`$`p[GG]`, 3)
      anova_result$`Sphericity Corrections`$`p[GG]`[anova_result$`Sphericity Corrections`$`p[GG]` < .001] <- "< .001"
    }
    
    if ("p[HF]" %in% names(anova_result$`Sphericity Corrections`)) {
      anova_result$`Sphericity Corrections`$`p[HF]` <- round(anova_result$`Sphericity Corrections`$`p[HF]`, 3)
      anova_result$`Sphericity Corrections`$`p[HF]`[anova_result$`Sphericity Corrections`$`p[HF]` < .001] <- "< .001"
    }
  }
...
# Formatierte Ausgabe
formatted_anova <- format_anova_results(anova_result)
print("\nANOVA Ergebnisse:")
print(formatted_anova)

# Erstellen einer kombinierten Bedingungsvariable
Bedingungen_data_Wirkungsgrad$Bedingung_Kombination <- paste(
  Bedingungen_data_Wirkungsgrad$Bedingung,
  Bedingungen_data_Wirkungsgrad$Intensität
)

### Verlgeich nur zwischen den 3 Bedingungen
# Definieren der relevanten Vergleiche
vergleiche <- list(
  c("sitzen leicht", "stehen leicht"),
  c("sitzen moderat", "stehen moderat"),
  c("sitzen schwer", "stehen schwer")
)
...
# Funktion zur Berechnung von Cohen's d für gepaarte Daten
...
# Durchführung der drei relevanten Vergleiche
for(vergleich in vergleiche) {
  x <- Bedingungen_data_Wirkungsgrad$`Wirk_Brutto [%]`[Bedingungen_data_Wirkungsgrad$Bedingung_Kombination == vergleich[1]]
  y <- Bedingungen_data_Wirkungsgrad$`Wirk_Brutto [%]`[Bedingungen_data_Wirkungsgrad$Bedingung_Kombination == vergleich[2]]
  
  test_result <- t.test(x, y, paired = TRUE)
  d <- cohens_d_paired(x, y)
  
  ergebnisse <- rbind(ergebnisse, data.frame(
    Vergleich = paste(vergleich[1], "vs", vergleich[2]),
    t_wert = test_result$statistic,
...
ergebnisse$p_wert_korrigiert <- p.adjust(ergebnisse$p_wert, method = "bonferroni")

# Effektstärke interpretieren
ergebnisse$effektstaerke_interpretation <- ifelse(
  abs(ergebnisse$cohens_d) < 0.2, "vernachlässigbar",
  ifelse(abs(ergebnisse$cohens_d) < 0.5, "klein",
         ifelse(abs(ergebnisse$cohens_d) < 0.8, "mittel", "groß")))

# Formatierung der Zahlen
ergebnisse$t_wert <- round(ergebnisse$t_wert, 2)
ergebnisse$p_wert <- round(ergebnisse$p_wert, 3)
ergebnisse$p_wert_korrigiert <- round(ergebnisse$p_wert_korrigiert, 3)
ergebnisse$cohens_d <- round(ergebnisse$cohens_d, 2)

# Sehr kleine p-Werte als < .001 darstellen
ergebnisse$p_wert[ergebnisse$p_wert < .001] <- "< .001"
ergebnisse$p_wert_korrigiert[ergebnisse$p_wert_korrigiert < .001] <- "< .001"

ergebnisse <- ergebnisse[, c("Vergleich", "t_wert", "p_wert", "p_wert_korrigiert", "cohens_d")]

print("Vergleiche der Bedingungen bei gleicher Intensität:")
print(ergebnisse)
...
# Post-hoc Ergebnisse erstellen
posthoc_ergebnisse_sitzen_Bedingung_Intensität_Wirk_muskulär <- NULL
...
stats$`Sitzen Leicht (MW ± SD)` <- sapply(stats$Variable, function(var) {
  ...
  if(var %in% c("Wirk_muskulär [%]","Wirk_Total [%]", "Wirk_Netto [%]", "Wirk_Brutto [%]")) {  
    ...
    if(var %in% c("Wirk_muskulär [%]", "Wirk_Netto [%]")) {
      ...
      posthoc_ergebnisse_sitzen_Bedingung_Intensität_Wirk_muskulär[[var]] <<- list(
...
stats$`Sitzen Moderat (MW ± SD)` <- sapply(stats$Variable, function(var) {
  ...
  } else if(var %in% c("Wirk_Arbeit [%]","Wirk_Total [%]", "Wirk_Brutto [%]", "Wirk_Netto [%]")) {
    sprintf("%.2f ± %.2f", 
            100 * mean(data_sitzen_moderate[[var]], na.rm=TRUE),
            100 * sd(data_sitzen_moderate[[var]], na.rm=TRUE))
  } else if(var %in% c("W_Aerob [kJ]", "W_PCR [kJ]", "W_BLC [kJ]", "W_TOT [kJ]",
...
# Post-hoc Ergebnisse erstellen
posthoc_ergebnisse_Stehen_Intensität_Wirk_Brutto <- NULL
posthoc_ergebnisse_Stehen_Intensität_Wirk_Netto <- NULL
...
stats$`Stehen Leicht (MW ± SD)` <- sapply(stats$Variable, function(var) {
  ...
  if(var %in% c("Wirk_muskulär [%]","Wirk_Total [%]", "Wirk_Netto [%]", "Wirk_Brutto [%]")) {
    ...
    # Berechnung der gepoolten Standardabweichung und Cohen's d
    sd_light <- sd(filter(temp_data, Intensität == "leicht")$dv)
    sd_moderate <- sd(filter(temp_data, Intensität == "moderat")$dv)
    sd_heavy <- sd(filter(temp_data, Intensität == "schwer")$dv)
    n <- length(filter(temp_data, Intensität == "leicht")$dv)
    
    pooled_sd <- sqrt(((n-1)*(sd_light^2 + sd_moderate^2 + sd_heavy^2))/(3*(n-1)))
    
    cohens_d_light_mod <- abs(mean(filter(temp_data, Intensität == "moderat")$dv) - 
                                mean(filter(temp_data, Intensität == "leicht")$dv)) / pooled_sd
    cohens_d_light_heavy <- abs(mean(filter(temp_data, Intensität == "schwer")$dv) - 
                                  mean(filter(temp_data, Intensität == "leicht")$dv)) / pooled_sd
    cohens_d_mod_heavy <- abs(mean(filter(temp_data, Intensität == "schwer")$dv) - 
                                mean(filter(temp_data, Intensität == "moderat")$dv)) / pooled_sd
    
    # Post-hoc Matrix erstellen und speichern
    post_hoc_matrix <- as.matrix(post_hoc_stehen$Intensität)
    post_hoc_matrix <- cbind(post_hoc_matrix, 
                             "Cohen's d" = c(cohens_d_light_mod, 
                                             cohens_d_light_heavy, 
                                             cohens_d_mod_heavy))
    
    # Speichern der Post-hoc Ergebnisse je nach Variable
    if(var == "Wirk_Brutto [%]") {
      posthoc_ergebnisse_Stehen_Intensität_Wirk_Brutto <<- list(Intensität = post_hoc_matrix)
    } else if(var == "Wirk_Netto [%]") {
      posthoc_ergebnisse_Stehen_Intensität_Wirk_Netto <<- list(Intensität = post_hoc_matrix)
    }
...
stehen_header <- as_paragraph("Stehen (MW ± SD)", as_sup(" ‡"))
...
ft_Wirkungsgrad_stats_Bedingung_Intensitaet_mean <- flextable(stats) %>%
  ...
  compose(
    ...
    i = 1,
    j = 2,
    value = as_paragraph("Sitzen (MW ± SD)")
  ) %>%
  compose(
    part = "header",
    i = 1,
    j = 5,
    value = as_paragraph("Stehen (MW ± SD)")
  ) %>%
  compose(
    j = "Variable",
  ...
  add_footer_row(
    values = as_paragraph(
      as_sup("a"), " : Parametrische Analyse mittels zweifaktorieller Varianzanalyse (ANOVA) mit Messwiederholung und anschließenden Post-hoc t-Tests mit Bonferroni-Korrektur; ",
      as_sup("†"), " : Signifikanter Interaktionseffekt der zweifaktoriellen ANOVA mit Messwiederholung (p < .05); ",
      "η", as_sub("muskulär"), " [%]: muskulärer Wirkungsgrad; ",
      "η", as_sub("Total"), " [%]: Gesamtwirkungsgrad; ",
      "η", as_sub("Arbeit,sitzen"), " [%]: Arbeitswirkungsgrad (sitzende Position); ",
      "η", as_sub("Netto"), " [%]: Nettowirkungsgrad; ",
...
### Manuelle Post-hoc Tabelle von Wirk_muskulär
post_hoc_data <- data.frame(
  Vergleich = c("Leicht: Sitzen vs. Stehen", 
                "Moderat: Sitzen vs. Stehen",
                "Schwer: Sitzen vs. Stehen"),
  Differenz = c(1.48, 0.84, 0.44),  # korrigierte Differenzen
  t_Wert = c(4.08, 2.29, 1.19),
  p_Wert = c(0.004, 0.051, 0.269),
  p_Wert_korrigiert = c(0.011, 0.153, 0.807),
  Cohens_d = c(1.36, 0.76, 0.40)
)
...
# Signifikanzsymbole hinzufügen
post_hoc_data$p_Wert_korrigiert <- paste0(
  format(round(post_hoc_data$p_Wert_korrigiert, 3), nsmall = 3),
  sapply(post_hoc_data$p_Wert_korrigiert, add_significance_symbols)
)

# Erstellen der Flextable
ft_post_hoc_muskulär <- flextable(post_hoc_data) %>%
  set_header_labels(
    Vergleich = " ",
    Differenz = "Mittlere Differenz",
    t_Wert = "t-Wert",
    p_Wert = "p-Wert",
    p_Wert_korrigiert = "p-Wert (Bonferroni korregiert)",
    Cohens_d = "Cohen's d"
  ) %>%
  compose(
    part = "header",
    i = 1, j = "Vergleich",
    value = as_paragraph("Post-hoc Vergleiche von η", as_sub("muskulär"))
  ) %>%
  theme_zebra(
    odd_header = "grey92",
    even_header = "grey92",
  ...
  border_inner_v(part = "all", border = fp_border(color = "darkgrey", width = 0.5)) %>%
  colformat_double(
    j = c("Differenz", "t_Wert", "p_Wert", "Cohens_d"),
    digits = 3
  )

# Setzen der Tabelle auf volle Breite
ft_post_hoc_muskulär <- set_table_properties(
  ft_post_hoc_muskulär,
  width = 1,
  layout = "autofit"
)
...
# Speichern in ...Probanden_Energieberechnung/xlsm
saveRDS(ft_post_hoc_muskulär, "C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Daten/Probanden_Energieberechnung/xlsm/ft_post_hoc_muskulär.rds")
ft_post_hoc_muskulär <- readRDS("C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Daten/Probanden_Energieberechnung/xlsm/ft_post_hoc_muskulär.rds")

# Speichern in ...Ergebnisse/rds
saveRDS(ft_post_hoc_muskulär, "C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad.github.io/Ergebnisse/rds/ft_post_hoc_muskulär.rds")
ft_post_hoc_muskulär <- readRDS("C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad.github.io/Ergebnisse/rds/ft_post_hoc_muskulär.rds")


############################################################################
### Manuelle Post-hoc Tabelle von Wirk_Netto
post_hoc_data <- data.frame(
  Vergleich = c("Leicht: Sitzen vs. Stehen", 
                "Moderat: Sitzen vs. Stehen",
                "Schwer: Sitzen vs. Stehen"),
  Differenz = c(0.19, -0.34, -0.62),  # korrigierte Differenzen
  t_Wert = c(0.46, -0.81, -1.60),
  p_Wert = c(0.659, 0.441, 0.148),
  p_Wert_korrigiert = c(1.000, 1.000, 0.445),
  Cohens_d = c(0.15, -0.27, -0.53)
)
...
# Erstellen der Flextable
ft_post_hoc_Netto <- flextable(post_hoc_data) %>%
  set_header_labels(
    Vergleich = " ",
    Differenz = "Mittlere Differenz",
    t_Wert = "t-Wert",
  ...
  compose(
    part = "header",  
    i = 1, j = "Vergleich",
    value = as_paragraph("Post-hoc Vergleiche von η", as_sub("Netto"))
  ) %>%
  theme_zebra(
    odd_header = "grey92",
    even_header = "grey92",
...
# Setzen der Tabelle auf volle Breite
ft_post_hoc_Netto <- set_table_properties(
  ft_post_hoc_Netto,  
  width = 1, 
  layout = "autofit"
)
...
# Speichern in ...Probanden_Energieberechnung/xlsm
saveRDS(ft_post_hoc_Netto, "C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Daten/Probanden_Energieberechnung/xlsm/ft_post_hoc_Netto.rds")
ft_post_hoc_Netto <- readRDS("C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Daten/Probanden_Energieberechnung/xlsm/ft_post_hoc_Netto.rds")

# Speichern in ...Ergebnisse/rds
saveRDS(ft_post_hoc_Netto, "C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad.github.io/Ergebnisse/rds/ft_post_hoc_Netto.rds")
ft_post_hoc_Netto <- readRDS("C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad.github.io/Ergebnisse/rds/ft_post_hoc_Netto.rds")


################################################################################################
### Manuelle Post-hoc Tabelle von Wirk_Brutto
post_hoc_data <- data.frame(
  Vergleich = c("Leicht: Sitzen vs. Stehen", 
                "Moderat: Sitzen vs. Stehen",
                "Schwer: Sitzen vs. Stehen"),
  Differenz = c(0.12, -0.33, -0.56),  # korrigierte Differenzen
  t_Wert = c(0.38, -0.92, -1.70),
  p_Wert = c(0.711, 0.386, 0.128),
  p_Wert_korrigiert = c(1.000, 1.000, 0.384),
  Cohens_d = c(0.13, -0.31, -0.57)
)
...
# Erstellen der Flextable
ft_post_hoc_Brutto <- flextable(post_hoc_data) %>%
  set_header_labels(
    Vergleich = " ",
    Differenz = "Mittlere Differenz",
    t_Wert = "t-Wert",
  ...
  compose(
    part = "header",
    i = 1, j = "Vergleich",
    value = as_paragraph("Post-hoc Vergleiche von η", as_sub("Brutto"))
  ) %>%
  theme_zebra(
    odd_header = "grey92",
    even_header = "grey92",
...
# Setzen der Tabelle auf volle Breite
ft_post_hoc_Brutto <- set_table_properties(
  ft_post_hoc_Brutto,
  width = 1,
  layout = "autofit"
)
...
# Speichern in ...Probanden_Energieberechnung/xlsm
saveRDS(ft_post_hoc_Brutto, "C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Daten/Probanden_Energieberechnung/xlsm/ft_post_hoc_Brutto.rds")
ft_post_hoc_Brutto <- readRDS("C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Daten/Probanden_Energieberechnung/xlsm/ft_post_hoc_Brutto.rds")

# Speichern in ...Ergebnisse/rds
saveRDS(ft_post_hoc_Brutto, "C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad.github.io/Ergebnisse/rds/ft_post_hoc_Brutto.rds")
ft_post_hoc_Brutto <- readRDS("C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad.github.io/Ergebnisse/rds/ft_post_hoc_Brutto.rds")
