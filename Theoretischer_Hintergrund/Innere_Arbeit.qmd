...
die auf theoretischen biomechanischen Modellen oder kinematischen Daten basieren. Im Folgenden werden ausgewählte Methoden vorgestellt und die angewandte Methodik detailliert erläutert.
...
Regression aus mehreren Belastungsstufen (@fig-Hintzy_Cloutier_linear), vergleichbar mit dem Verfahren der
Deltawirkungsgradberechnung [@Gaesser1975; @Francescato1995; @Sjøgaard2002].<br>
Dabei werden wie in @fig-Hintzy_Cloutier_linear zu sehen verschiedene Werte des Sauerstoffvolumenstroms im Bezug zur dafür absolvierten mechanischen Leistung für verschiedene Intensitäten in einem Diagramm dargestellt und eine lineare Regression durchgeführt. Die Regressionsgleichung lautet dabei: $\Delta\dot{V}O_2 (\Delta P_{mech}) = a + b \cdot \Delta P_{mech}$<br>
Der y-Achsenabschnitt (a) dieser Regression entspricht dem Sauerstoffvolumenstrom bei 0 Watt $\Delta\dot{V}O_{2,\text{unloaded}}$. 

::: {.callout-note collapse="true"}
#### Beispielberechnung des Delta-Wirkungsgrads
Bei der Berechnung des Delta-Wirkungsgrads wird dann die Steigung der Regressionsgeraden (b) durch Umrechnung mittels des kalorischen Äquivalents (abhängig vom RQ-Wert) in den Delta-Wirkungsgrad transformiert [@Francescato1995; @Gaesser1975; @Zoladz2023a; @Ettema2009].

1. Gegebene Parameter:
   - Steigung b = 11.65 ml·O~2~·kg^-1^·W^-1^
   - RQ = 0.97 → kalorisches Äquivalent = 20.87 kJ·l·O~2~^-1^

2. Umrechnung auf ml O~2~:
   $\frac{20.87}{1000} = 0.02087$ [kJ·ml·O~2~^-1^]

3. Multiplikation mit der Steigung:
   $11.65 \cdot 0.02087 = 0.243$ [kJ·kJ^-1^]

4. Kehrwert für Delta-Wirkungsgrad:
   $\Delta\eta = \frac{1}{0.243} = 0.247$

5. Umrechnung in Prozent:
   $\Delta\eta = 0.247 \cdot 100 = 24.7\%$

$\Delta\eta = \frac{1}{11.65 \cdot \frac{20.87}{1000}} \approx 0.247 = 24.7\%$
:::

Die Validität dieses linearen Modells wurde jedoch kritisch diskutiert [@Ettema2009a], da oberhalb der ersten Laktatschwelle[^1]
...
ansteigender Last vorgeschlagen (@fig-Hintzy_Cloutier_curvilinear) [@Hintzy-Cloutier2003].<br>
...
:::::: columns
::: {.column width="49%"}
![Lineare Beziehung zwischen P~mech~ und Sauerstoffvolumenstrom. VO~2,unload~ zeigt den y-Achsenabschnitt der linearen ΔVO~2~-ΔP~mech~-Regression (Hintzy-Cloutier, 2003).](images/Hintzy-Cloutier_linear.png){#fig-Hintzy_Cloutier_linear}
:::
...
::: {.column width="49%"}
![Kurvilineare Beziehung zwischen P~mech~ und Sauerstoffvolumenstrom. VO~2,unload~ zeigt den y-Achsenabschnitt der kurvilinearen ΔVO~2~-ΔP~mech~-Regression (Hintzy-Cloutier, 2003).](images/Hintzy-Cloutier_curvilinear.png){#fig-Hintzy_Cloutier_curvilinear}
:::
::::::   
...
> $W_{\text{Int \kern0.05em pro \kern0.05em Umdrehung}}$  = 0.153 · 70 · (1.33)^3^ = 25.39 \[J\] \  |   Umdrehungen<sub>gesamt</sub> = 1.33 · 300 = 400 \[U\]<br> 
...
unterschied. Dabei wurde der menschliche Bewegungsapparat als segmentiertes Modell dargestellt, indem der Körper in biomechanisch relevante Segmente für die Analyse des Gangzyklus unterteilt wurde. @Winter1979
...
Zunächst wurde der Kurbelwinkel φ~1~(t) bestimmt, der sich linear mit der Zeit änderte:
...
- Die durchschnittliche innere Leistung wurde aus der Summe der positiven Leistungsanteile beider Beine berechnet:<br> $\bar{P}_{Int} = \overline{P_{Int,Links,pos}(t)} + \overline{P_{Int,Rechts,pos}(t)}$

Im weiteren Verlauf dieser Arbeit wird die modellbasiert berechnete innere Leistung als P~Int,Modell~[^1] bezeichnet.

- Die innere Arbeit (W~Int~ in Kilojoule) über den definierten Belastungszeitraum wurde aus der mittleren inneren Leistung (P~Int~ in Watt) und der Belastungsdauer (t in Sekunden) berechnet:<br> $W_{Int} = \frac{\bar{P}_{Int} \cdot t}{1000}$

[^1]: Hinweis zur Notation: In den textlichen Beschreibungen werden die Größen P~Int,Modell~, P~Int,Kinematik~, P~Int,Kinematik_Modell~ usw. der Einfachheit halber ohne Überstrich dargestellt. Dabei handelt es sich jedoch um die arithmetischen Mittelwerte über dem jeweiligen Belastungszeitraum. Eine explizite Zeitabhängigkeit wird durch die Notation (t) gekennzeichnet, wie beispielsweise bei P~Int,Modell~(t). Diese Notationsweise wird auch in den folgenden Abschnitten für die hier beschriebenen Parameter beibehalten.
...
Diese Diskrepanz steht im Widerspruch zu den berechneten Wirkungsgraden (η~Total~, η~Netto~, η~Brutto~, alle ohne Berücksichtigung der inneren Leistung) sowie den erhobenen physiologischen Parametern des durchschnittlichen Sauerstoffvolumenstroms ($\dot{V}O_2$) und der durchschnittlichen Herzrate. Da diese Parameter bei vergleichbarer mechanischer Leistung keine signifikanten Unterschiede zwischen stehender und sitzender Position aufwiesen, lässt sich die Hypothese ableiten, dass die innere Leistung in beiden Körperpositionen ähnliche Werte aufweisen sollte.<br>
Aufgrund der fehlenden Bestätigung dieser Annahme durch die gewählten Berechnungsmethoden wurde nachträglich ein alternativer Berechnungsansatz untersucht. Dieser basiert auf dem Modell von @Winter1979 und wurde gemäß der Methodik von @Hansen2004 für die spezifischen Anforderungen des Radfahrens adaptiert. Die wesentlichen methodischen Erweiterungen gegenüber dem zuvor beschriebenen Berechnungsweg für P~Int,Kinematik,Modell~ umfassten drei zentrale Aspekte: Erstens die Integration der potentiellen Energie der Segmente, zweitens die Berücksichtigung des HAT-Segments (Head-Arms-Trunk) als zusammenhängende biomechanische Einheit und drittens die Annahme eines vollständigen Energietransfers zwischen den Segmenten über die Grenzen der einzelnen Gliedmaßen hinweg.
