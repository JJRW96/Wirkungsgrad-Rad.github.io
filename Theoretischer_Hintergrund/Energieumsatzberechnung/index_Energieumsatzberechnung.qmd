...
Es gibt zwei Ansätze zur Bestimmung des Energieertrags pro verstoffwechselter Sauerstoffeinheit. Der klassische Ansatz verwendet die indirekte Kalorimetrie, um den Sauerstoffvolumenstrom sowie die Kohlensstoffdioxidabgabe während der Belastung zu bestimmen und ermittelt anhand standardisierter Tabellen den Energieumsatz in kcal ⋅ min^-1^ oder kJ ⋅ min^-1^. Ein anderer Ansatz berechnet auf Basis des Sauerstoffvolumenstroms die ADP-zu-ATP-Phosphorylierungsrate unter Verwendung von Annahmen aus der Biochemie. Letztere Methode wird als "theoretisch-thermodynamischer Ansatz" (im Original: "theoretical-thermodynamic approach") bezeichnet und wurde erstmals von @Whipp1969 verwendet. Da beide Methoden sehr ähnliche Ergebnisse liefern und die klassische Methode einfacher anzuwenden ist sowie zusätzlich Einblicke in die Verteilung der Energiesubstrate ermöglicht, wurde sie für die folgenden Berechnungen verwendet [@Brooks2012]. In folgendem Abschnitt werden die Grundlagen der Energieumsatzberechnung sowie der indirekten Kalorimetrie beschrieben: 
...
Der Proteinstoffwechsel wird in den durchgeführten Berechnungen nicht berücksichtigt. 
