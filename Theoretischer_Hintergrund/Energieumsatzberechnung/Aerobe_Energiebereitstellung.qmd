...
Um den aeroben Energienteil am physiologischen Gesamtenergieumsatz (W~AER~) zu bestimmen muss der Sauerstoffvolmenstrom über den Belastungszeitraum ($\dot{V}O_{2, Bel}$) und der Sauerstoffvolumenstom in Ruhe bestimmt werden ($\dot{V}O_{2, Ruhe}$).<br>
Die Ruhesauerstoffaufnahme kann etweder in Ruhe direkt gemessen werden direkt gemessen werden oder mit Hilfe von Gleichungen, wie beispielsweise @eq-H und @eq-RMR, berechnet werden. @eq-H beschreibt die Wärmeproduktion (h) in 24 Stunden in kcal für Frauen und Männer [@Harris1918]:
...
Nach der Berechnung der Basalrate, kann mithilfe des vor der Belastung für jeden Probanden gemessenen RQ und @tbl-stegemann das kalorischen Äquivalents (kÄ) bestimmt werden und der Ruheumsatz (RMR , Englisch für "resting metabolic rate") bzw. der Sauerttoffvolumenstrom in RUhe in im Liegen in Liter pro Minute  berechnet werden. Um RMR bzw. im Liegen auf den Ruheumsatz im Stehen zu kommen, wird der berechnete Ruheumsatz mit dem Faktor $\frac{1.39}{1.08} = 1.287$ für Männer und $\frac{1.15}{0.90} = 1.278$ für Frauen, basierend auf @fig-CIBA_GEIGY multipliziert [@Ciba-Geigy1977, 225]. Der mit folgender Gleichung berechneter RMR entspricht dem berechneten $\dot{V}O_{2, Ruhe}$:
...
Parameter: Körpermasse = 75 kg; Körperlänge = 180 cm; Alter = 25 Jahre; RQ = 0.77 entspricht kÄ von 19.946 (siehe @tbl-stegemann).
