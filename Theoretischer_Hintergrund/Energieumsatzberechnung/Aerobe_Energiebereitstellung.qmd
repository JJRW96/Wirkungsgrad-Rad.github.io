...
$h_{m} = 66,5 + (13,75 \cdot \text{75}) + (5,003 \cdot \text{180}) - (6,775 \cdot \text{25}) = 1828,915\, [\text{kcal}]$<br> $RMR_{m} = \left( \frac{1828,915}{24 \cdot 60 \cdot 19,946} \right) \cdot 4,1868 \cdot 1,287 = 0,344\, [l \cdot min^{-1}]$
:::

.

In der voliegenden Studie wurde die Ruhesauerstoffaufnahme sowohl durch eine direkte Messung 10 Minuten vor dem Start der ersten Belastung auf dem Fahrradergometer in sitzender Position gemessen, als auch durch die eben beschribene Berechnungsmethode bestimmt. Da aufgrund verschiedeneer Faktoren, wie bswp. durch Stress, Nahrungsaufnahme oder verschiedene Umgebungsfaktoren, die Werte der gemessenen Ruhesauerstoffaufnahme unnatürlich hoch sien können im Vergleich zu den Normwerte, wurde die gemessene $\dot{V}O_{2,Ruhe}$ nur verwendet, wenn sie maximale 20% größer war als die berechnete. Sonst wurde der berechnete Wert verwendet. 

## Berechnung der aeroben Energiekomponente W~AER~

Um den aeroben Arbeitsumsatz (W~aer~) eines Belastungszeitraumes zu erhalten muss zunächst $\dot{V}O_{2,Netto}\;[l \cdot min^{-1}]$ wie folgt bestimtm werden:

$$
\dot{V}O_{2,Netto}\;[l \cdot min^{-1}] = \dot{V}O_{2,Bel}\, [l \cdot min^{-1}] - \dot{V}O_{2,Ruhe}\,[l \cdot min^{-1}]
$$

muss der Ruheenergieumsatz (W~Ruhe~) vom errechneten aeroben Anteil des Gesamtenergieumsatzes abgezogen werden. 
